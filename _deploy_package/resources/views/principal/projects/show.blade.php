...
<div class="space-y-6">
            ...
            <div class="flex items-center gap-3">
                @if($project->project_status === 'completed')
                    <form method="POST" action="{{ route('principal.projects.approve-closure', $project->projectID) }}">
                        @csrf
                        <button type="submit" class="px-4 py-2 bg-emerald-600 text-white rounded-md hover:bg-emerald-700 transition-colors" onclick="return confirm('Approve closure and archive this project?')">
                            Approve Closure
                        </button>
                    </form>
                @endif
                <a href="{{ route('principal.projects.edit', $project->projectID) }}" class="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-colors">Edit Project</a>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="bg-white rounded-lg shadow p-6">
            ...
            <div class="w-full bg-gray-200 rounded-full h-4 overflow-hidden">
                <div class="h-full rounded-full transition-all duration-500 ease-out {{ $latestProgress >= 100 ? 'bg-green-500' : ($latestProgress >= 50 ? 'bg-blue-500' : 'bg-amber-500') }}" style="width: {{ min($latestProgress, 100) }}%">
                    @if($latestProgress >= 15)<span class="text-[10px] text-white font-bold flex items-center justify-center h-full">{{ number_format($latestProgress, 1) }}%</span>@endif
                </div>
            </div>
        ...
        <div class="bg-white rounded-lg shadow p-6">
            ...
            <div class="w-full bg-gray-200 rounded-full h-4 overflow-hidden">
                <div class="h-full rounded-full transition-all duration-500 ease-out {{ $fundProgress >= 100 ? 'bg-green-500' : ($fundProgress >= 50 ? 'bg-emerald-500' : 'bg-amber-500') }}" style="width: {{ min($fundProgress, 100) }}%">
                    @if($fundProgress >= 15)<span class="text-[10px] text-white font-bold flex items-center justify-center h-full">{{ number_format($fundProgress, 1) }}%</span>@endif
                </div>
            </div>
        ...
        <div class="bg-white rounded-lg shadow p-6">
            ...
            <div class="w-full bg-gray-200 rounded-full h-4 overflow-hidden">
                <div class="h-full bg-purple-500 rounded-full transition-all duration-500 ease-out" style="width: {{ $milestoneProgress }}%">
                    @if($milestoneProgress >= 15)<span class="text-[10px] text-white font-bold flex items-center justify-center h-full">{{ number_format($milestoneProgress, 1) }}%</span>@endif
                </div>
            </div>
            <p class="text-xs text-gray-500 mt-2">{{ $totalMilestones === 0 ? 'No milestones defined yet' : number_format($milestoneProgress, 1) . '% milestones completed' }}</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow p-6">
        ...
        <div id="milestone-form" class="{{ $errors->any() ? '' : 'hidden' }} mb-6 bg-gray-50 rounded-lg p-4 border border-gray-200">
            ...
            @endif
            <form method="POST" action="{{ route('principal.projects.milestones.store', $project->projectID) }}" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                @csrf
                <div>
                ...
                <div class="space-y-4">
                    @foreach($milestones as $milestone)
                        <div class="relative flex items-start gap-4 pl-12">
                            <div class="absolute left-3 top-1 w-5 h-5 rounded-full border-2 flex items-center justify-center {{ $milestone->is_completed ? 'bg-purple-500 border-purple-500' : 'bg-white border-gray-300' }}">
                            ...
                            <div class="flex-1 bg-gray-50 rounded-lg p-4 border {{ $milestone->is_completed ? 'border-purple-200 bg-purple-50' : 'border-gray-200' }}">
                                <div class="flex items-center justify-between flex-wrap gap-2">
                                    ...
                                    <div class="flex items-center gap-2">
                                        <form method="POST" action="{{ route('principal.projects.milestones.toggle', [$project->projectID, $milestone->milestoneID]) }}">
                                            @csrf
                                            <button type="submit" class="text-xs px-3 py-1 rounded-md border transition-colors {{ $milestone->is_completed ? 'border-gray-300 text-gray-600 hover:bg-gray-100' : 'border-purple-300 text-purple-600 hover:bg-purple-50' }}">
                                                {{ $milestone->is_completed ? 'Undo' : 'Complete' }}
                                            </button>
                                        </form>
                                        <form method="POST" action="{{ route('principal.projects.milestones.destroy', [$project->projectID, $milestone->milestoneID]) }}">
                                            @csrf
                                            @method('DELETE')
                                ...
                                <div class="flex items-center gap-4 mt-2 text-xs text-gray-500">
                                    @if($milestone->target_date)<span>Target: {{ $milestone->target_date->format('M d, Y') }}</span>@endif
                                    @if($milestone->completed_date)<span class="text-green-600">Completed: {{ $milestone->completed_date->format('M d, Y') }}</span>@endif
                                </div>
                            </div>
                        </div>
                    @endforeach
    ...
    <div class="bg-white rounded-lg shadow p-6">
        ...
        <form method="POST" action="{{ route('principal.projects.updates.store', $project->projectID) }}">
            @csrf
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="number" step="0.01" min="0" max="100" name="progress_percentage"
                            ...
                            oninput="document.getElementById('progress-preview-pr').style.width = Math.min(this.value || 0, 100) + '%'">
                        <span class="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 text-sm">%</span>
                    </div>
                    <div class="w-full bg-gray-200 rounded-full h-1.5 mt-2">
                        <div id="progress-preview-pr" class="h-full bg-blue-500 rounded-full transition-all duration-300" style="width: {{ min($latestProgress, 100) }}%"></div>
                    </div>
                </div>
            ...
            @forelse ($updates as $update)
                <div class="border border-gray-200 rounded-lg p-4 hover:border-gray-300 transition-colors">
                    ...
                    <div class="mt-3 flex items-center gap-3">
                        <div class="flex-1">
                            <div class="w-full bg-gray-200 rounded-full h-2">
                                <div class="h-full rounded-full transition-all duration-300 {{ $update->progress_percentage >= 100 ? 'bg-green-500' : ($update->progress_percentage >= 50 ? 'bg-blue-500' : 'bg-amber-500') }}" style="width: {{ min($update->progress_percentage, 100) }}%"></div>
                            </div>
                        </div>
                ...
                <tbody class="divide-y divide-gray-200">
                    @forelse ($contributions as $contribution)
                        <tr>
                            ...
                            <td class="px-4 py-2 text-sm text-gray-700">{{ $contribution->parent ? $contribution->parent->first_name . ' ' . $contribution->parent->last_name : 'Unknown Parent' }}</td>
                            ...
                            <td class="px-4 py-2 text-sm text-gray-700">
                                @if($contribution->receipt_number)
                                    <a href="{{ route('principal.contributions.receipt', $contribution->contributionID) }}" target="_blank" class="text-green-600 hover:text-green-700 text-sm font-medium">Print</a>
                                @else
                    ...
                    @empty
                        <tr><td colspan="5" class="px-4 py-4 text-sm text-gray-500 text-center">No contributions recorded.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
