...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-lg shadow p-6">
        <div class="flex items-center justify-between flex-wrap gap-4">
            ...
            <div class="flex items-center gap-3">
                ...
                @endif
                <a href="{{ route('teacher.projects.edit', $project->projectID) }}" class="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-colors">Edit Project</a>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow p-6">
        ...
        <div id="milestone-form" class="{{ $errors->any() ? '' : 'hidden' }} mb-6 bg-gray-50 rounded-lg p-4 border border-gray-200">
            ...
            @endif
            <form method="POST" action="{{ route('teacher.projects.milestones.store', $project->projectID) }}" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                @csrf
                <div>
                            ...
                            <div class="flex-1 bg-gray-50 rounded-lg p-4 border {{ $milestone->is_completed ? 'border-purple-200 bg-purple-50' : 'border-gray-200' }}">
                                <div class="flex items-center justify-between flex-wrap gap-2">
                                    ...
                                    <div class="flex items-center gap-2">
                                        <form method="POST" action="{{ route('teacher.projects.milestones.toggle', [$project->projectID, $milestone->milestoneID]) }}">
                                            @csrf
                                        ...
                                        <form method="POST" action="{{ route('teacher.projects.milestones.destroy', [$project->projectID, $milestone->milestoneID]) }}">
                                            @csrf
                                            @method('DELETE')
    ...
    <div class="bg-white rounded-lg shadow p-6">
        ...
        <form method="POST" action="{{ route('teacher.projects.updates.store', $project->projectID) }}">
            @csrf
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="number" step="0.01" min="0" max="100" name="progress_percentage"
                            ...
                            oninput="document.getElementById('progress-preview-te').style.width = Math.min(this.value || 0, 100) + '%'">
                        <span class="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 text-sm">%</span>
                    </div>
                    <div class="w-full bg-gray-200 rounded-full h-1.5 mt-2">
                        <div id="progress-preview-te" class="h-full bg-blue-500 rounded-full transition-all duration-300" style="width: {{ min($latestProgress, 100) }}%"></div>
                    </div>
                </div>
                        ...
                        <tr>
                            ...
                            <td class="px-4 py-2 text-sm text-gray-700">
                                @if($contribution->receipt_number)
                                    <a href="{{ route('teacher.payments.receipt', $contribution->contributionID) }}" target="_blank" class="text-green-600 hover:text-green-700 text-sm font-medium">Print</a>
                                @else
