            ...
            <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                ...
                <div>
                    <div class="flex items-center gap-1">
                        ...
                        <?php else: ?>
                            <a href="<?php echo e($users->appends(request()->query())->previousPageUrl()); ?>" class="px-3 py-1 text-sm text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50 transition-colors">&lt;</a>
                        <?php endif; ?>

                        <?php for($page = max(1, $users->currentPage() - 2); $page <= min($users->lastPage(), $users->currentPage() + 2); $page++): ?>
                            ...
                            <?php else: ?>
                                <a href="<?php echo e($users->appends(request()->query())->url($page)); ?>" class="px-3 py-1 text-sm text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50 transition-colors"><?php echo e($page); ?></a>
                            <?php endif; ?>
                        <?php endfor; ?>

                        <?php if($users->hasMorePages()): ?>
                            <a href="<?php echo e($users->appends(request()->query())->nextPageUrl()); ?>" class="px-3 py-1 text-sm text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50 transition-colors">&gt;</a>
                        <?php else: ?>
                            <span class="px-3 py-1 text-sm text-gray-400 bg-gray-100 rounded-md cursor-not-allowed">&gt;</span>
                        <?php endif; ?>
                    </div>
        ...
        <div class="px-6 py-4 border-b border-gray-200 bg-gradient-to-r from-blue-50 to-indigo-50">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                ...
                <div class="flex gap-2">
                    <form method="POST" action="<?php echo e(route($routePrefix . '.students.import')); ?>" enctype="multipart/form-data" class="inline-flex">
                        <?php echo csrf_field(); ?>
                ...
                <div>
                    <div class="flex items-center gap-1">
                        <?php if($students->onFirstPage()): ?>
                            <span class="px-3 py-1 text-sm text-gray-400 bg-gray-100 rounded-md cursor-not-allowed">&lt;</span>
                        <?php else: ?>
                            <a href="<?php echo e($students->appends(request()->query())->previousPageUrl()); ?>" class="px-3 py-1 text-sm text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50 transition-colors">&lt;</a>
                        <?php endif; ?>

                        <?php for($page = max(1, $students->currentPage() - 2); $page <= min($students->lastPage(), $students->currentPage() + 2); $page++): ?>
                            <?php if($page == $students->currentPage()): ?>
                                <span class="px-3 py-1 text-sm font-semibold text-white bg-green-600 rounded-md"><?php echo e($page); ?></span>
                            <?php else: ?>
                                <a href="<?php echo e($students->appends(request()->query())->url($page)); ?>" class="px-3 py-1 text-sm text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50 transition-colors"><?php echo e($page); ?></a>
                            <?php endif; ?>
                        <?php endfor; ?>

                        <?php if($students->hasMorePages()): ?>
                            <a href="<?php echo e($students->appends(request()->query())->nextPageUrl()); ?>" class="px-3 py-1 text-sm text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50 transition-colors">&gt;</a>
                        <?php else: ?>
                            <span class="px-3 py-1 text-sm text-gray-400 bg-gray-100 rounded-md cursor-not-allowed">&gt;</span>
                        <?php endif; ?>
                    </div>
...
<?php echo $__env->make($layout ?? 'layouts.ad-sidebar', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\Web Developement\JCES-PTA and Project Management System\resources\views/administrator/users.blade.php ENDPATH**/ ?>